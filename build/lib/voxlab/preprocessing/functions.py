...
def convert_to_mono(audio, method='left'):
    ...
    method (str): The method to convert to mono ('left', 'right', 'mean').
    ...
    if method == 'left':
        ...
        mono_samples = samples.mean(axis=1).astype(samples.dtype)
    else:
        raise ValueError(f"Unsupported method: {method}. Use 'left', 'right', or 'mean'.")

    mono_audio = AudioSegment(
        mono_samples.tobytes(),
...
def remove_silence(audio, silence_thresh=-30, min_silence_len=1000, min_segment_len=1000, fade_duration=50,
    ...
    for segment in non_silent_segments:
        ...
        output_audio += segment + AudioSegment.silent(duration=silence_duration)
